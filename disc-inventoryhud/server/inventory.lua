...
AddEventHandler("disc-inventoryhud:GiveCash", function(data)
    ...
    if data.item == 'cash' then
        if sourcePlayer.getMoney() >= data.count then
		    ...
		    TriggerClientEvent('mythic_notify:client:SendAlert', targetPlayer, { type = 'inform', text = sourcePlayer ..' Memberikan uang sebanyak ' .. data.count })
            TriggerClientEvent('disc-inventoryhud:refreshInventory', source)
            TriggerClientEvent('disc-inventoryhud:refreshInventory', data.target)
            sendToDiscord(16753920, "NORTHS RP", "**" .. GetPlayerName(source) .. "** Memberikan uang sebanyak **" .. data.count .. "** kepada **" .. targetPlayer .. "**", "All Seeing Eye")
        end
    ...
    elseif data.item == 'black_money' then
        if sourcePlayer.getAccount('black_money').money >= data.count then
            ...
            sendToDiscord(16753920, "NORTHS RP", "**" .. GetPlayerName(source) .. "** Memberikan uang **merah** sebanyak **" .. data.count .. "** kepada **" .. targetPlayer .. "**", "All Seeing Eye")
            TriggerClientEvent('disc-inventoryhud:refreshInventory', source)
            TriggerClientEvent('disc-inventoryhud:refreshInventory', data.target)
        end
    end
...
function sendToDiscord(color, name, message, footer)
  ...
  PerformHttpRequest('https://discordapp.com/api/webhooks/707590994517819405/Mo3akAY-GJrWpRVGGHAHowGan0NLQURjafO1G-o978OmHliETU5nTee9yMq3s47A_bYz', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end